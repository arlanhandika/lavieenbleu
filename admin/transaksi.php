...
if(isset($_SESSION['valdi'])) {
	if (isset($_POST['update'])) {
		...
		if ($status == 'Sedang Diantar') {
			...
			$update = mysqli_query($koneksi, "UPDATE transaksi SET status='$status', no_resi='$no_resi' WHERE id_transaksi='$id_transaksi'");
                					...
                					<input type="hidden" name="id_transaksi" value="<?php echo $obj->id_transaksi ?>">
										<tr>
											...
											<td>
												...
												<?php }
												elseif ($obj->status == 'Sedang Dikemas') { ?>
															<select name="status" class="form-select">
																<option selected><?php echo $obj->status ?></option>
												...
												<?php }
												elseif ($obj->status == 'Sedang Diantar') { ?>
															<select name="status" class="form-select">
																<option selected><?php echo $obj->status ?></option>
																<option value="Verifikasi Pembayaran">Verifikasi Pembayaran</option>
																<option value="Sedang Dikemas">Sedang Dikemas</option>
												...
												<?php }
												elseif ($obj->status == 'Pesanan Berhasil') { ?>
															<select name="status" class="form-select">
																<option selected><?php echo $obj->status ?></option>
																<option value="Verifikasi Pembayaran">Verifikasi Pembayaran</option>
																<option value="Sedang Dikemas">Sedang Dikemas</option>
												...
												<?php }
												elseif ($obj->status == 'Pesanan Gagal') { ?>
															<select name="status" class="form-select">
																<option selected><?php echo $obj->status ?></option>
																<option value="Verifikasi Pembayaran">Verifikasi Pembayaran</option>
																<option value="Sedang Dikemas">Sedang Dikemas</option>
												...
												<?php }
												elseif ($obj->status == 'Pesanan Batal') { ?>
															<select name="status" class="form-select">
																<option selected><?php echo $obj->status ?></option>
																<option value="Verifikasi Pembayaran">Verifikasi Pembayaran</option>
																<option value="Sedang Dikemas">Sedang Dikemas</option>
											...
											if ($obj->status == 'Sedang Diantar') { 
												if ($obj->no_resi == '') { ?>
													<input type="text" name="no_resi" placeholder="Masukkan No. Resi"></input>
												<?php }
												else {
													echo $obj->no_resi;
												} 
